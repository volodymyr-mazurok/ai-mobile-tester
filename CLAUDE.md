...
Native, Xcode projects or your app's source; you need a `.apk` or a Simulator `.app`.

⚠️ **This is the `main` branch: the boilerplate. There is no app here**, no specs, no
requirements and no findings - `SCREENS` is empty and `config/app.ts` holds
placeholders. Everything below still applies; it just has nothing to run against yet.
The **`demo`** branch is the same framework wired to a real app, with two suites, an
exploratory charter and a real bug it finds - read it when you want a worked example
of any file here.
...
npm run setup              # reminds you where the app binary goes (see docs/guides/building-the-app.md)
